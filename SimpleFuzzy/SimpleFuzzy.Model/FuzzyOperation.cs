...
namespace SimpleFuzzy.Model
{
    public class FuzzyOperation : IMembershipFunction
    {
        ...
        public static Dictionary<string, (bool, Func<IMembershipFunction, IMembershipFunction, object, double, double>)> operations = new Dictionary<string, (bool, Func<IMembershipFunction, IMembershipFunction, object, double, double>)>()
        {
            ...
            // Бинарные операции
            {"Нечеткое \"И\"", (false, (x, y, z, p) => p * Math.Min(x.MembershipFunction(z), y.MembershipFunction(z)) + 0.5 * (1 - p) * (x.MembershipFunction(z) + y.MembershipFunction(z)))},
            {"Нечеткое \"ИЛИ\"", (false, (x, y, z, p) => p * Math.Max(x.MembershipFunction(z), y.MembershipFunction(z)) + 0.5 * (1 - p) * (x.MembershipFunction(z) + y.MembershipFunction(z)))},
